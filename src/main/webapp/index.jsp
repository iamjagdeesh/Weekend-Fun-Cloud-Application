...
function mapFun(src, dst) {
	console.log(src);
	var mapSrcContent = "https://www.google.com/maps/embed/v1/directions?key=AIzaSyDKf_wK_7lVCODXoD6cS_tOdMrmTchxxZo&origin="+src+"&destination="+dst+"&avoid=tolls";
...
function imageFun(link) {
	var imageSrcContent = "https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&key=AIzaSyCjzdF82OByGEz2cxfQeL-hITA2D0w31Hc&photoreference="+link;
...
<body onLoad="initialize('<%=state1%>','<%=groupType%>','<%=budgetType%>','<%=address%>','<%=dateSelected%>')" bgcolor="#cce0ff">
	<div class="container-fluid">
		...
		<div class="row">
				...
				"Wyoming" };
					...
					<div class="form-group">
						<label for="groupType">Select group type:</label> <select
							class="form-control" id="groupType" name="groupType" required="true">
					...
					<div class="form-group">
						<label for="budgetType">Type of budget:</label> <select
							class="form-control" id="budgetType" name="budgetType" required="true">
					...
					<div class="form-group">
						<label for="selectedDate">Fun day to choose:</label> <input
							...
							name="selectedDate" max="2018-05-08" min="2018-05-04" required="true">
					</div>
			...
			<div class="col-sm-4">
				...
				<h3>Graphics</h3>
				<div class="row">
					<iframe id="mapFrame" width="90%" height="35%" frameborder="1" style="border: 1; display: none;"
					...
					<input type="button" id="mapButton" class="button" value="Show/Hide Map">
				</div>
				<div class="row">
					<img id="imageFrame" width="90%" height="35%" frameborder="1" style="border: 1; display: none;"
					...
					allowfullscreen> </img>
					<input type="button" id="imageButton" class="button" value="Show/Hide Image">
				</div>
			</div>
		</div>
	</div>
